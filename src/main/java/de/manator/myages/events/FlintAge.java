...
public class FlintAge implements Listener {
	...
	public void onBlockBreak(org.bukkit.event.block.BlockBreakEvent e) {
		...
		if (usedItem.hasItemMeta() && usedItem.getItemMeta().getLore() != null
				&& usedItem.getItemMeta().getLore().contains("Flint Pickaxe")) {
			if (block.getType() == Material.IRON_ORE || block.getType().toString().contains("DEEPSLATE")
					|| block.getType() == Material.COAL_ORE || block.getType() == Material.COPPER_ORE) {
